...
maybe_start_http_listener() ->
    case spartan_config:http_enabled() of
        true ->
            IPs = spartan_config:bind_ips(),
...
start_http_listener(IP) ->
    Dispatch = cowboy_router:compile([
        {'_', [
            {"/v1/version", spartan_http_handler, [version]},
            {"/v1/config", spartan_http_handler, [config]},
            {"/v1/hosts/:host", spartan_http_handler, [hosts]},
            {"/v1/services/:service", spartan_http_handler, [services]},
            {"/v1/enumerate", spartan_http_handler, [enumerate]},
            {"/v1/records", spartan_http_handler, [records]}
        ]}
    ]),
    Port = spartan_config:http_port(),
    cowboy:start_http(
        IP, 1024,
        [{port, Port}, {ip, IP}],
