...
handle_packet_cache_miss(Message, [], Host) ->
  {ok, Response} = spartan_dns_dual_dispatch_fsm:sync_resolve(Message, [], Host),
...
safe_handle_packet_cache_miss(Message, AuthorityRecords, Host) ->
  case application:get_env(erldns, catch_exceptions) of
    {ok, false} ->
      {ok, Response} = spartan_dns_dual_dispatch_fsm:sync_resolve(Message, AuthorityRecords, Host),
      % Response = erldns_resolver:resolve(Message, AuthorityRecords, Host),
      maybe_cache_packet(Response, Response#dns_message.aa);
    _ ->
      try spartan_dns_dual_dispatch_fsm:sync_resolve(Message, AuthorityRecords, Host) of
